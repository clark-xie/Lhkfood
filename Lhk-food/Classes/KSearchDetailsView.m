...
@interface KSearchDetailsView ()

@end

@implementation KSearchDetailsView
...
@synthesize mulGraphics=_mulGraphics;
...
- (void)didTapButton:(UIButton *)sender
{
    ...
    int n=0;
    n = pathToCell.row;
    //AGSPoint *point = [self.multiPoint objectAtIndex:n];
    //[self.custemCallout putDataSet:[self getDataSet]];
    //self.mapView.callout.customView = self.custemCallout.view;
    
    NSInteger m = 10*(self.curPage-1)+n;
    NSLog(@"当前要素%d",m);
    ...
    [backGroundView putQuery:YES];
    [backGroundView zoomToPoint:[self.multiPoint objectAtIndex:n] withLevel:12];
    //[backGroundView.custemCallout putGraphic:[self.mulGraphics objectAtIndex:n]];
    // backGroundView.mapView.callout.customView = self.mainController.custemCallout.view;
    //[backGroundView.mapView showCalloutAtPoint:point forGraphic:[self.mulGraphics objectAtIndex:n] animated:YES];
    UINavigationController *nav = [[UINavigationController alloc] initWithRootViewController:backGroundView];
    [self presentModalViewController:nav animated: YES];

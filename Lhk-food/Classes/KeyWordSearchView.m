...
-(void)initScrollView{
    self.scrollView=[[UIScrollView alloc]initWithFrame:CGRectMake(0, 90, 540, 568)];//初始化scrollview的界面 （坐标x，坐标y，宽度，高度）屏幕左上角为原点
    [self.scrollView setContentSize:CGSizeMake(1080, 768)];//设置scrollview画布的大小，此设置为三页的宽度，380的高度。用来实现三页照片的转换。
    ...
    self.pageControl=[[MyPageControl alloc]initWithFrame:CGRectMake(210, 31, 60, 36)];
...
//初始化搜索历史
-(void)initPage1{
    UIView *subview1 = [[UIView alloc] initWithFrame:CGRectMake(0, 2, 540, 528)];
    subview1.backgroundColor = [UIColor clearColor];
    self.tableView=[[UITableView alloc] initWithFrame:CGRectMake(0, 2, 540, 528)];
    ...
    UIView *labelview = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 540, 44)];
    ...
    [button setFrame:CGRectMake(25, -1, 330, 46)];
    [labelview addSubview:button];
    
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(150, 0, 540, 44)];
    label.text = @"清空搜索历史";
...
#define ROWHEIHT 50
    ...
    for (int i=0; i<total; i++) {
        ...
        UIView *subview2 = [[UIView alloc] initWithFrame:CGRectMake(65*column+550, ROWHEIHT*row, 40, ROWHEIHT)];
...
#define ROWHEIHT 50
    ...
    for (int i=total; i<self.cates.count; i++) {
        ...
        UIView *subview3 = [[UIView alloc] initWithFrame:CGRectMake(80*column+970, ROWHEIHT*row, 40, ROWHEIHT)];
